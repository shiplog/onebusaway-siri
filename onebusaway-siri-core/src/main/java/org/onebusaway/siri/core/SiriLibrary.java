...
public class SiriLibrary {
    ...
    public static <T extends AbstractServiceRequestStructure> List<T> getServiceRequestsForModule(
        ...
        switch (moduleType) {
            ...
            case ESTIMATED_TIMETABLE:
                return (List<T>) serviceRequest.getEstimatedTimetableRequest();
            default:
    ...
    public static <T extends AbstractSubscriptionStructure> List<T> getSubscriptionRequestsForModule(
        ...
        switch (moduleType) {
            ...
            case ESTIMATED_TIMETABLE:
                return (List<T>) subscriptionRequest.getEstimatedTimetableSubscriptionRequest();
            default:
                return new ArrayList<T>();
        }
    }
    ...
    public static <T extends AbstractServiceDeliveryStructure> List<T> getServiceDeliveriesForModule(
        ...
        switch (moduleType) {
            ...
            case ESTIMATED_TIMETABLE:
                return (List<T>) serviceDelivery.getEstimatedTimetableDelivery();
            default:
                return new ArrayList<T>();
        }
    }
     ...
     * publication windows and validity periods of the situation all ending
     * before the current time. If a situation has no publication window or
     * validity periods, it is considered active.
     *
     * @param situation
     * @param currentTime
    ...
    public static boolean isSituationExpired(
        ...
        /**
         * If the situation has no publication window or valid periods defined,
         * we consider the alert to be active.
         */
    ...
    /**
     * Returns true if the specified time range is active at the specified time
     * or will become active at some point in the future. We do not consider an
     * empty time range to ever be active.
     *
     * @param range
     * @param time
     * @return true if the specified time range is active at the specified time
     * or will become active at some point in the future
     */
    public static boolean isTimeRangeActiveOrUpcoming(
            HalfOpenTimestampRangeStructure range, Date time) {
        /**
         * We do not consider an empty time range to be active or upcoming.
         * Technically, the range should always have a start time anyway, but
         * just in case.
         */
        if (range.getStartTime() == null && range.getEndTime() == null) {
            return false;
